...
client.on('message', async message => {
        ...
        if (hasPermission(message)) {
            ...
            if (!member.kickable) {
                return message.reply('eu não posso expulsar este usuário! Ele pode ter um cargo mais alto ou eu ' +
                    'não tenho permissões de expulsa-lo.');
            }
            ...
            await member.kick(reason).catch(error => message.reply(`Desculpe ${message.author} não consegui 
            expulsar o membro devido: ${error}.`));
    ...
    // Esse comando remove um número x de mensagens dos usuários no canal, limitado a no máximo 100 mensagens
    ...
    // COMANDOS TEAM LIQUIDO
        ...
        message.channel.send('Digite um comando existente rapariga \nPara ajuda digite $help');
    }
